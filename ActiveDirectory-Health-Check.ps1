...
if ($useBpa) {
    ...
    Invoke-BpaModel -ModelId Microsoft/Windows/DirectoryServices | Out-Null
...
# check if AD DS is running
if ((Get-Service NTDS).status -eq "Running") {
    $nagStates.Set_Item("ADDS", 0)
    $nagExitString += "AD DS service is running. "
}
else {
    $nagStates.Set_Item("ADDS", 2)
    $nagExitString += "AD DS service IS NOT running. "
}

# test error - comment out the next two lines in production
